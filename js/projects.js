fetch("/assets/web/projects.json")
  .then((res) => res.json())
  .then((data) => {
    const displayOptionSetting = localStorage.getItem("project-display");

    const sliderOptionBtn = document.querySelector(
      ".display-btns.display-slider"
    );
    ...
    showFavBtn.addEventListener("click", (e) => {
      ...
      if (!toggleFav) {
        ...
        const favouriteList = JSON.parse(
          localStorage.getItem("favourite_projects")
        );

        for (let i = 0; i < allGridCards.length; i++) {
          if (favouriteList[i] == false) allGridCards[i].style.display = "none";
          else if (favouriteList[i] == true)
            allGridCards[i].style.display = "inherit";
        }
      } else if (toggleFav) {
        toggleFav = false;
